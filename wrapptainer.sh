...
get_apptainer_cmd() {
    if [ $ignore_command == false ]; then
        if [ $run_type == "exec" ]; then
            apt_cmd="apptainer exec${tool_special:+ }$tool_special $bind_dirs $tool_img $tool $args"
        elif [ $run_type == "run" ]; then
            apt_cmd="apptainer run${tool_special:+ }$tool_special $bind_dirs $tool_img $tool $args"
        elif [ $run_type == "shell" ]; then
            apt_cmd="apptainer shell${tool_special:+ }$tool_special $bind_dirs $tool_img"
        fi
    elif [ $ignore_command == true ]; then
        if [ $run_type == "exec" ]; then
            apt_cmd="apptainer exec${tool_special:+ }$tool_special $bind_dirs $tool_img $args"
        elif [ $run_type == "run" ]; then
            apt_cmd="apptainer run${tool_special:+ }$tool_special $bind_dirs $tool_img $args"
        elif [ $run_type == "shell" ]; then
            apt_cmd="apptainer shell${tool_special:+ }$tool_special $bind_dirs $tool_img"
        fi
    fi
